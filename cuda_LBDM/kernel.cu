...
__device__ void makePivotMatrix(double *pivot_arr, int *pivot_vec)
{
	int pc[len9];
...
__device__ void assignLUMatrix(double *A, double *L, double *U)
{
	//获取下三角矩阵L
...
__device__ void assignXiMatrix(double *imdata, double *Xi, int tid, int cols, int x, int y )
{
...
__device__ void solve(double **A, double *B, int *pivot_vec, double *pivot_arr, int *info, double *L, double *U)
{
	double alpha = 1.0f;
	double beta = 0.0f;
	...
	alpha = 0.0f;
	beta = 1.0f;
	status = cublasDgeam(handle, CUBLAS_OP_N, CUBLAS_OP_T,
		len9, len9,
		&alpha,
		A[0], len9,
		&beta, B, len9,
	...
	alpha = 0.0f;
	beta = 1.0f;
	status = cublasDgeam(handle, CUBLAS_OP_N, CUBLAS_OP_N,
		len9, len9,
		&alpha,
		B, len9,
		&beta, A[0], len9,
...
__device__ void mapDeviceMem(
	double *d_Xi, double *d_A, double **d_pA, double *d_B, int *d_pivot_vec, double *d_pivot_arr, int *d_info, double *d_L, double *d_U, 
	int *d_trimap, int *d_row_inds, int *d_col_inds, double *d_vals,
 	//
	double *&Xi, double **&A, double *&B, int *&pivot_vec, double *&pivot_arr, int *&info, double *&L, double *&U,
...
__device__ void createDiagnal(double *I, double val, int len)
{
	unsigned sz = sizeof(double)* len * len;
...
__device__ void compLapcoeff(double *Xi, double **A, double *B, int *pivot_vec, double *pivot_arr, int *info, double *L, double *U){
	//
	double lambda = 0.000001f;
...
//__global__ void kernel(double *d_Xi, double *d_A, double **d_pA, double *d_B, int *d_pivot_vec, double *d_pivot_arr, int *d_info, double *d_L, double *d_U)
//{
...
__global__ void kernel_new(double *d_Xi, double *d_A, double **d_pA, double *d_B, int *d_pivot_vec, double *d_pivot_arr, int *d_info, double *d_L, double *d_U,
...
void allocMem(
	int pixNum,
	double *&d_Xi, double **&d_pA, double *&d_A, double *&d_B, int *&d_pivot_vec, double *&d_pivot_arr, int *&d_info, double *&d_L, double *&d_U,
	double *&d_imdata, double *&d_vals, int *&d_trimap, int *&d_row_inds, int *&d_col_inds)
{
	//d_Xi
	...
	//d_pA和d_A
	checkCudaErrors(cudaMalloc((void**)&d_pA, pixNum * sizeof(double *)));
	...
	checkCudaErrors(cudaMalloc((void**)&d_pivot_arr, pixNum * len99 * sizeof(double)));

	//d_info
	checkCudaErrors(cudaMalloc((void**)&d_info, pixNum * sizeof(int)));

	//L和U
	checkCudaErrors(cudaMalloc((void**)&d_L, pixNum * len99 * sizeof(double)));
	checkCudaErrors(cudaMalloc((void**)&d_U, pixNum * len99 * sizeof(double)));
...
void freeMem(double *&d_Xi, double **&d_pA, double *&d_A, double *&d_B, int *&d_pivot_vec, double *&d_pivot_arr, int *&d_info, double *&d_L, double *&d_U,
	double *&d_imdata, double *&d_vals, int *&d_trimap, int *&d_row_inds, int *&d_col_inds)
{
	cudaFree(d_Xi);
	cudaFree(d_pA);
...
extern "C" void callKernel(int rows, int cols, double *imdata, int *trimap, int *row_inds, int *col_inds, double *vals)
{
	int pixNum = rows * cols;

	double *d_Xi, *d_A, **d_pA, *d_B, *d_pivot_arr, *d_L, *d_U;
	...
	//double xi[len9*len9];
	//checkCudaErrors(cublasGetMatrix(len9, len4, sizeof(double), d_Xi, len9, xi, len9));
	//checkCudaErrors(cublasGetMatrix(len9, len9, sizeof(double), d_A, len9, a, len9));
	//checkCudaErrors(cublasGetMatrix(len9, len9, sizeof(double), d_B, len9, b, len9));
	//checkCudaErrors(cublasGetMatrix(len9, len9, sizeof(double), d_L, len9, l, len9));
	//checkCudaErrors(cublasGetMatrix(len9, len9, sizeof(double), d_U, len9, u, len9));
	//checkCudaErrors(cublasGetMatrix(len9, len9, sizeof(double), d_pivot_arr, len9, p, len9));
...
//
//	double xi[len94], a[len99], b[len99], l[len99], u[len99], p[len99];
//
//	//
//	double *d_Xi, *d_A, **d_pA, *d_B;
//	int *d_pivot_vec, *d_info;
//	double *d_pivot_arr;
//	double *d_L, *d_U;
//	//
//	allocMem(d_Xi, d_pA, d_A, d_B, d_pivot_vec, d_pivot_arr, d_info, d_L, d_U);
//
//	for (int i = 0; i < len94; ++i)
//	{
//		xi[i] = Xi[i];
//	}
//	
//	checkCudaErrors(cublasSetMatrix(len9, len4, sizeof(double), xi, len9, d_Xi, len9));
//
//	//checkCudaErrors(cublasSetMatrix(len9, len9, sizeof(double), a, len9, d_A, len9));
//	//checkCudaErrors(cublasSetMatrix(len9, len9, sizeof(double), b, len9, d_B, len9));
//	//checkCudaErrors(cublasGetMatrix(len9, len4, sizeof(double), d_Xi, len9, xi, len9));
//
//	kernel << <128, 1 >> >(d_Xi, d_A, d_pA, d_B, d_pivot_vec, d_pivot_arr, d_info, d_L, d_U);
//
//	//checkCudaErrors(cublasGetMatrix(len9, len4, sizeof(double), d_Xi, len9, xi, len9));
//	checkCudaErrors(cublasGetMatrix(len9, len9, sizeof(double), d_A, len9, a, len9));
//	checkCudaErrors(cublasGetMatrix(len9, len9, sizeof(double), d_B, len9, b, len9));
...
//	//double timeSpan;
//	//checkCudaErrors(cudaEventElapsedTime(&timeSpan, start, stop));
...
//}
//
//__device__ void mapDeviceMem(float *d_Xi, float *d_A, float **d_pA, float *d_B, int *d_pivot_vec, float *d_pivot_arr, int *d_info, float *d_L, float *d_U,
//	float *&Xi, float **&A, float *&B, int *&pivot_vec, float *&pivot_arr, int *&info, float *&L, float *&U, int tid)
//{
//	Xi = &d_Xi[tid*len94];
//
//	A = &d_pA[tid];
//	d_pA[tid] = &d_A[tid*len99];
//
//	//float *data = &d_A[tid*len99];
//
//	//if (tid == 0)
//	//{
//	//	data[0] = 1;		data[3] = 2;		data[6] = 6;
//	//	data[1] = 4;		data[4] = 8;		data[7] = -1;
//	//	data[2] = -2;		data[5] = 3;		data[8] = -5;
//	//}
//	//else
//	//{
//	//	data[0] = 1;		data[3] = 2;		data[6] = 6;
...
////=========================================================================================================================================================//
//
//void allocMem(float *&d_Xi, float **&d_pA, float *&d_A, float *&d_B, int *&d_pivot_vec, float *&d_pivot_arr, int *&d_info, float *&L, float *&U)
//{
//	//d_Xi
//	checkCudaErrors(cudaMalloc((void**)&d_Xi, effThreads * len94 * sizeof(float)));
//
//	//d_A和d_pA
//	checkCudaErrors(cudaMalloc((void**)&d_A, effThreads * len99 * sizeof(float)));
//	checkCudaErrors(cudaMalloc((void**)&d_pA, effThreads * sizeof(float *)));
//
//	//d_pivot和d_info
//	checkCudaErrors(cudaMalloc((void**)&d_pivot_vec, effThreads * len9 * sizeof(int)));
//	checkCudaErrors(cudaMalloc((void**)&d_pivot_arr, effThreads * len99 * sizeof(float)));
//	checkCudaErrors(cudaMalloc((void**)&d_info, effThreads * sizeof(int)));
//
//	//d_B
//	checkCudaErrors(cudaMalloc((void**)&d_B, effThreads * len99* sizeof(float)));
//
//	//L和U
//	checkCudaErrors(cudaMalloc((void**)&L, effThreads * len99 * sizeof(float)));
//	checkCudaErrors(cudaMalloc((void**)&U, effThreads * len99 * sizeof(float)));
//
//}
//
...
//
//
//extern "C" int m(float* Xi, float *res)
//{
//	//cudaEvent_t start, stop;
//	//checkCudaErrors(cudaEventCreate(&start));
//	//checkCudaErrors(cudaEventCreate(&stop));
//	//checkCudaErrors(cudaEventRecord(start, 0));
//
//	float xi[len94], a[len99], b[len99], l[len99], u[len99], p[len99];
//
//	//
//	float *d_Xi, *d_A, **d_pA, *d_B;
//	int *d_pivot_vec, *d_info;
//	float *d_pivot_arr;
//	float *d_L, *d_U;
//	//
...
//	}
//
//	checkCudaErrors(cublasSetMatrix(len9, len4, sizeof(float), xi, len9, d_Xi, len9));
